<template>
  <div id="balance">
    <div id="head">
      <div id="section">
        <div id="line1">
          <span id="baye">当前积分</span>
          <img src="../../assets/minePicture/ques.png" alt=""><span id="bades" @click="toAboutIntegral">积分说明</span>
          <div id="empty"></div>
        </div>
        <div id="line2"><span id="moneynow">{{balance}}</span>元</div>
        <span id="withdraw" :style="withdrawbtn" @click="tomine">积分兑换商品</span>
      </div>
    </div>
    <p id="des">最近30天积分记录</p>
    <div id="bynothing">
      <img src="../../assets/minePicture/buynothing.png" alt="">
      <p>最近30天无积分记录</p>
      <p id="moreintegral">快去下单赚取大量积分吧</p>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    beforeCreate() {
      this.$store.commit("updateCharacter","我的积分");
   ...
   methods:{
     ...
     toAboutIntegral(){
       this.myHttp.get(this.myApi.myApi.servercenter,(data)=>{
         this.$router.push({path:"/aboutintegral",query:{pointtextContent:data.pointtextContent,}})
       },(err)=>{
         alert(err)
       })
     }
