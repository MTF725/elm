<template>
  <!-- 搜索送餐地址页面   路由是  /search_delivery_address   -->
  <div id="search-address">
    <!--<div class="search-address-head">-->
      <!--<img src="../../../../assets/img/prev.png" @click="prevPage" />-->
      <!--<span>搜索送餐地址</span>-->
    <!--</div>-->
    ...
    <div class="search-result">
      <ul>
        <router-link :to="{path:'/add-delivery-address',query:{selectAddress:this.inputV}}" class="router-link-active">
          <li v-for="(itmes,index ) in getAdressData" :key="index" @click="inputV = itmes.address " class="search-result-ul-li">
            <p>{{itmes.name}}</p>
            <p>{{itmes.address}}</p>
          </li>
        </router-link>
...
<script>
  export default {
    ...
    created(){
      ...
      this.$store.commit("updateCharacter","搜索送餐地址");
